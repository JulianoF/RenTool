...
#### Linux
For debain based systems, simply run (with suffient permissions if needed ```sudo```):
```
...
```
Then edit your **system** environment varables *not user environment variables* by setting the variable **JAVA_HOME** to the newly unpacked folder, and appending java to your system path if not already present.
```
...
Docker is used to run and orchestrate the containers that RenTool runs in. This is done for cross platfrom compatability and for development/deployment consistency between developers/machines.

#### Linux
For debian based systems, simply run (with suffiet permissions if needed ```sudo```):
```bash
...
Windows machines need to have wsl (Windows subsystem Linux) installed/enabled to install docker. If it is not already installed/enabled, run the following in an administrator priviledged command prompt or power shell:
```powershell
wsl --install
```
...
#### Linux
Use *SDKMAN!* to install Gradle. There are other methods but they have a tendency to install older, outdated versions. Follow the directions [here](https://sdkman.io/install) to install *SDKMAN!*. Then run the following (with suffient permissions if needed ```sudo```):
```bash
...
```
Then edit your **system** environment varables *not user environment variables* by setting the variable **GRADLE_HOME** to the newly unpacked folder, and appending gradle to your system path if not already present.
```
